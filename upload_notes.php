...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array($fileType, $allowedTypes)) {
        // ... (existing code for file upload and database connection)

        $db_name = 'mysql:host=localhost;dbname=nms';
        $user_name = 'root';
        $user_password = '';
        ...
        try {
            ...
            $sql = "INSERT INTO notes_uploadd (branch, role, name, subject, topic, filename, filetype, description, status)
                    VALUES (:branch, :role, :name, :subject, :topic, :filename, :filetype, :description, 'pending')";
...
<script>
  ...
  function validateForm(event) {
    ...
    const notesfile = form.elements["notesfile"].value;
    const filetype = form.elements["filetype"].value;
    const description = form.elements["description"].value.trim();
    const name = form.elements["name"].value.trim(); // New field - use trim() to remove leading/trailing whitespace

    // Create an object to store specific error messages for each field
    const errorMessages = {
      branch: "Department is not selected.",
      role: "Role is not selected.",
      subject: "Course is not selected.",
      topic: "Topic is not filled.",
      notesfile: "Notes file is not selected.",
      filetype: "File type is not selected.",
      description: "Description is not filled.",
      name: "Name is not filled.", // New field - error message for the name field
    };

    // Create an array to store the names of fields with missing values
    const missingFields = [];

    // Check each required field and add its name to the missingFields array if it's empty
    if (!branch) missingFields.push("branch");
    if (!role) missingFields.push("role");
    if (!subject) missingFields.push("subject");
    if (!topic) missingFields.push("topic");
    if (!notesfile) missingFields.push("notesfile");
    if (!filetype) missingFields.push("filetype");
    if (!description) missingFields.push("description");
    if (!name) missingFields.push("name"); // New field - check if name is empty

    // If any required fields are empty, prevent form submission and display the error messages
    if (missingFields.length > 0) {
      event.preventDefault();

      // Create the error message based on the missing fields
      let errorMessage = "The following fields are not filled:\n";
  ...
  // Add a "submit" event listener to the form that calls the validateForm function
